...
{
    //Volvemos a asignar el manejador de la señal SIGCHLD
    ...
    {
        //Cuando acabe el proceso hijo, actualizamos la lista de trabajos
    ...
    if (WIFEXITED(status))
    {
        sprintf(mensaje, "El proceso con PID: %d ha terminado con normalidad: %d\n", pid, WEXITSTATUS(status));
        write(2, mensaje, strlen(mensaje));
        fflush(stdout);
    }
    else if (WIFSIGNALED(status))
    {
        sprintf(mensaje, "\nEl proceso con PID: %d ha recibido la señal: %d\n", pid, WTERMSIG(status));
        write(2, mensaje, strlen(mensaje));
        fflush(stdout);
    }
}
...
int internal_export(char **args)
{
    ...
    if(var==NULL){
        printf(NEGRITA ROJO"Error de sintaxis. export: el nombre es nulo\n" RESET);
        return -1;
    }
...
int execute_line(char *line)
{
    ...
    if (exCheckInternal == 0)
    {
        ...
        // Si es el proceso hijo, ejecutamos el comando y hacemos que omita la señal SIGINT
        if (exFork == 0)
        {
