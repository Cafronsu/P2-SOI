...
/*
    ...
    Parametros: line (char*) - buffer donde se guardará la línea leída
...
char *read_line(char *line)
{
    ...
    char *linea = fgets(line, COMMAND_LINE_SIZE, stdin);
    if (linea != NULL)
    {
        size_t len = strlen(linea);
        if (len > 0 && linea[len - 1] == '\n')
        {
            linea[len - 1] = '\0'; // Eliminamos solo si es salto de línea
        }
        return linea;
    }
    else
...
{
    //Por ahora solo parseamos y comprobamos si es interno
